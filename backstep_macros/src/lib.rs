...
pub fn system(input: TokenStream) -> TokenStream {
    ...
    let struct_fields = storage_fields
        ...
        .map(|(field_name, ty, _mutability, is_mut)| {
            if *is_mut {
                quote! { pub #field_name: *mut backstep::storage::Storage<#ty> }
            } else {
                quote! { pub #field_name: *const backstep::storage::Storage<#ty> }
            }
        });
    ...
    let new_storage_init = storage_fields.iter().map(|(field_name, ty, _, is_mut)| {
        if *is_mut {
            ...
            quote! {
                let #field_name = {
                    world.get_storage::<#ty>() as *const backstep::storage::Storage<#ty>
                };
            }
        }
    });
    ...
    let param_gathering: Vec<_> = params
        ...
        .map(|(i, (param_name, _ty, is_mut))| {
            ...
            if *is_mut {
                ...
                quote! {
                    let mut #param_name = backstep::view::ViewMut::new(
                        #chunk_var,
            ...
            quote! {
                {
                    ...
                    if ((*self.#name).rollback.tick() != ct) {
                        let new_current = if let Some(mut pooled) = (*self.#name).rollback_pool.pop() {
                            ...
                            Box::new(backstep::rollback::RollbackStorage::with_tick(ct))
                        };
    ...
    let parent_impl = if let Some(pt) = parent_type {
        quote! {
            fn parent(&self) -> Option<&dyn backstep::system::SystemGroup> {
                Some(<#pt as backstep::system::SystemGroup>::instance())
            }
        }
    } else {
        quote! {}
    ...
    let expanded = quote! {
        ...
        impl #system_name {
            pub fn new(world: &mut backstep::world::World) -> Self {
                unsafe {
                    #(#new_storage_init)*
        ...
        impl backstep::system::System for #system_name {
            fn run(&self, _frame: &backstep::frame::Frame) {
                unsafe {
                    #(#rollback_inits)*
...
pub fn system_group(input: TokenStream) -> TokenStream {
    ...
    let after_ids: Vec<_> = after_types
        ...
        .map(|ty| quote! { std::any::TypeId::of::<#ty>() })
        .collect();
    let parent_static_ident = format_ident!("__backstep_group_parent_{}", group_name);
    ...
    let self_static_ident = format_ident!("__backstep_group_instance_{}", group_name);
    let expanded = quote! {
        pub struct #group_name;
        unsafe impl Send for #group_name {}
        unsafe impl Sync for #group_name {}
        static #self_static_ident: #group_name = #group_name;
        impl backstep::system::SystemGroup for #group_name {
            fn instance() -> &'static dyn backstep::system::SystemGroup where Self: Sized { &#self_static_ident }
            ...
            fn parent(&self) -> Option<&dyn backstep::system::SystemGroup> { #parent_expr }
